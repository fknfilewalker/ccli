...
namespace
{
	...
	void basicBoolTest()
	{
		...
		try {
			...
			std::cout << "Caught error: " << e.message() << '\n';
		}
	...
	void exceptionTest() {
...

		{
			...
			catch (const ccli::UnknownArgError& e) {
				didCatch = true;
	...
	void registeredVarTest()
	{
		...
		std::cout << "Currently registerd variables..." << '\n';
		ccli::forEachVar([](ccli::VarBase& var, const size_t idx) -> ccli::IterationDecision {
			std::cout << "... " << idx << " - " << var.longName() << " " << var.valueString() << '\n';
			return {};
			});

		std::cout << *float2Var.asBool() << '\n';
		std::cout << *float2Var.asInt() << '\n';
		std::cout << *float2Var.asFloat() << '\n';
		std::cout << float2Var.asString().has_value() << '\n';
		std::cout << *stringVar.asString() << '\n';
	}
	...
	void unregisteredVarWarning()
	{
		try {
			const char* argv[] = { "-b1", "1", "-b2", "true", "-b3", "0", "-b4", "false" };
			ccli::parseArgs(std::size(argv), argv);
		}
		catch (ccli::UnknownArgError& e)
		{
			assert(e.message() == "Unknown input '-b1' while parsing arguments.");
		}
		catch (ccli::CCLIError&) {
			assert(false);
		}
	}
}
